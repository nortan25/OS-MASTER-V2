...
<body>
    <div>
        <h1>Relatórios de Caixa Lucro</h1>
    ...
    document.getElementById('reportForm').addEventListener('submit', function(event) {
        ...
        fetch('{{ route('relatorios.getProfitReportData') }}', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            const values = data.map(item => parseFloat(item.profit));
            ...
            currentChart = new Chart(ctx, {
                type: 'bar', // Tipo de gráfico alterado para colunas (bar)
                data: {
                    labels: labels,
                    datasets: [{
                        label: 'Lucro',
                        data: values,
                        backgroundColor: values.map(value => value < 0 ? 'rgba(255, 99, 132, 0.2)' : 'rgba(75, 192, 192, 0.2)'),
                        borderColor: values.map(value => value < 0 ? 'rgba(255, 99, 132, 1)' : 'rgba(75, 192, 192, 1)'),
                        borderWidth: 1
                    }]
                },
                options: {
